    ...
    while (name.length < 3) {
      console.log(
        chalk.redBright.bold(
          "Please choose a longer name (3 or more characters)",
        ),
      );
...
let playerName = readlineSync.question(
  chalk.greenBright.bold("Enter your name: "),
);
...
const computer = {
  ...
  genName() {
    const names = [
      ...
      "Tim",
      "Bob",
      "Bloopo Fizzwizzle the Splendid",
      "Cassandra Bowman",
      "Sam",
      "Lisp",
      "Mocha",
  ...
  get intro() {
    return chalk.yellowBright.bold(
      `Hello, ${player.userName}! My name is ${this._name} and I will be your oponent! A little bit about me: \n * I have a ${this.degree} \n * My favorite food is ${this.favFood}`,
    );
  },
  get favFood() {
...
function victoryScreen(winningPlayer) {
  ...
  console.log(
    chalk.blueBright.bold("_________________________________________________"),
  );
  console.log(
    chalk.magentaBright.bold(
      `${winningPlayer === player.userName ? player.userName : computer.name} has won the game!`,
    ),
  );
  if (winningPlayer === player.userName) {
    ...
    console.log(
      chalk.magentaBright.bold(
        `Great job ${player.userName}! You win.... ${chalk.yellowBright("NOTHING!!")}`,
      ),
    );
  } else {
...
function mainGame() {
  ...
  console.log(
    chalk.magenta.bold(` ${player.userName}'s current score: ${player.score}`),
  );
  console.log(
    chalk.magenta.bold(` ${computer.name}'s current score: ${computer.score}`),
  );

  while (player._score > 0 && computer._score > 0) {
    let userMoveIndex = readlineSync.keyInSelect(
      ["Rock", "Paper", "Scissors"],
      chalk.greenBright.bold(`What's your move ${player.userName}?`),
    );
    ...
    if (rpsLogic[userMove] == computer._move) {
      ...
      console.log(
        chalk.magentaBright.bold(`${chalk.bgWhite(player.userName)} won!`),
      );
      console.log(
        chalk.magentaBright.bold(
          ` ${player.userName} chose: ${chalk.blueBright.bold(userMove)}\n ${computer.name} chose: ${computer.move}\n`,
        ),
      );
      computer.scoreDecrement();
      console.log(
        chalk.magentaBright.bold(
          ` ${player.userName}'s current score: ${player.score}\n ${computer.name}'s current score: ${computer.score}`,
        ),
      );
      console.log(compFaces.angryFace);
      ...
      console.log(
        chalk.magentaBright.bold(`${chalk.bgBlue(computer.name)} won!`),
      );
      console.log(
        chalk.magentaBright.bold(
          ` ${player.userName} chose: ${chalk.blueBright.bold(userMove)}\n ${computer.name} chose: ${computer.move}\n`,
...
console.log(compFaces.happyFace);
console.log(
  chalk.yellowBright.bold("Now let's get started! Here are the basic rules: "),
);
console.log(
  chalk.blueBright.bold("_________________________________________________"),
);
console.log(
  chalk.yellowBright.bold(
    `Both of our scores will begin at ${chalk.blueBright.bold("5")} and it will decrease by one each round one of us ${chalk.redBright.bold("loses")}. \nWhoever reaches a score of ${chalk.redBright.bold("0")} will ${chalk.redBright.bold("lose")} the game.`,
  ),
);
...
readyUp === false
  ? (console.log(
      chalk.yellowBright.bold("Why even run the program if you're not ready?"),
    ),
    console.log(compFaces.angryFace),
    process.exit(1))
  : mainGame();

...
let customGreet = (i) => {
    ...
    if (j) {
        ...
        console.log(
            `Alright that's enough smalltalk for today ${i}, bye bye now.`
        );
    } else if (
        j === !'good' ||
        j === !'Good' ||
        ...
        console.log(
            `Alright that's enough smalltalk for today ${i}, leave now.`
        );
    }
    ...
    if (h === 'yes' || h === 'Yes' || h === 'Yes.' || h === 'yes.') {
        console.log(
            `Me too! Wait looks like my programmer didn't leave any more lines for me to say, so good smalltalk, bye now ${i}`
        );
    }

...
let usrChoice = readlineSync.keyInSelect(
    matChoice,
    chalk.greenBright.bold('What matrix size do you want to work with? ')
);
usrChoice === 2
    ? process.exit(0)
    : console.log(
          chalk.yellowBright.bold(
              `Initializing ${matChoice[usrChoice]} matrix creation process`
          )
      );
...
function createMat(arr) {
    ...
    for (let i = 1; i <= usrMatID; i++) {
        usrMatRows = readlineSync.question(
            chalk.greenBright.bold(
                `Enter row number ${i} of your ${matChoice[usrChoice]} matrix: `
            )
        );
        ...
        if (usrMatRows.length != 3) {
            console.log(
                chalk.redBright.bold(
                    `ERROR: Only 3 numbers can be inputted for a ${matChoice[usrChoice]} matrix.`
                )
            );
            process.exit(1);
        }
...
console.log(
    chalk.yellowBright.bold(
        `Initiliazing ${matChoice[usrChoice]} matrix calculations: ${'\n'}`
    )
);
const calculations = [
    `Determinant of a ${matChoice[usrChoice]} matrix`,
    `Multiplication of two ${matChoice[usrChoice]} matrices`,
    `Subtracion of two ${matChoice[usrChoice]} matrices`,
    `Addition of two ${matChoice[usrChoice]} matrices`,
    `Scalar of a ${matChoice[usrChoice]} matrix`,
    'EXIT'
];
...
while (usrCalcChoice) {
    usrCalcChoice = readlineSync.keyInSelect(
        calculations,
        chalk.greenBright.bold('Enter the operation: ')
    );
    ...
    switch (usrCalcChoice) {
        ...
        case 4:
            {
                let scalarNum = readlineSync.questionInt(
                    chalk.greenBright.bold('Enter the constant: ')
                );
                console.log(chalk.greenBright.bold('The scalar matrix is: '));
                matScalar(usrMat, scalarNum).forEach((i) =>
                    console.log(chalk.magentaBright.bold(i + '\n'))
                );
            }
            break;
        case 5:
